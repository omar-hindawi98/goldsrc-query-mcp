...
export function registerTools(server: McpServer) {
	server.registerTool(
		...
		{
			title: "Ping",
			description: "Measure round-trip latency to a GoldSrc server. Use to check if a server is reachable and responsive.",
			inputSchema: baseSchema,
			annotations: readOnly,
		},
		({ address: host, port: p, timeout: t }) =>
			safely(() => withQuery(host, p, t, (q) => q.ping()).then((ms) => text(`${ms}ms`))),
	);
	...
	server.registerTool(
		"get_server_info",
		{
			...
			description: "Get server metadata: name, map, player counts, VAC status, and more. Use to get an overview of a server or check if it is online.",
			inputSchema: baseSchema,
			annotations: readOnly,
		},
		({ address: host, port: p, timeout: t }) =>
	...
	server.registerTool(
		...
		{
			title: "Get Players",
			description: "Get the current player list with names, scores, and time in-game. Use to see who is on a server.",
			inputSchema: baseSchema,
			annotations: readOnly,
		},
		({ address: host, port: p, timeout: t }) =>
	...
	server.registerTool(
		...
		{
			title: "Get All",
			description: "Get server info, player list, and rules in one call. Use this instead of calling get_server_info, get_players, and get_rules separately.",
			inputSchema: baseSchema,
			annotations: readOnly,
		},
		({ address: host, port: p, timeout: t }) =>
	...
	server.registerTool(
		...
		{
			title: "Send RCON",
			description: "Send a single RCON command and return the response. Use for one-off commands; prefer send_rcon_batch when sending multiple commands.",
	...
	server.registerTool(
		"send_rcon_batch",
		{
			...
			description: "Send multiple RCON commands over a single authenticated connection. Use instead of rcon when running more than one command.",
